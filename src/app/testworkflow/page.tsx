...
import { MapPin, ExternalLink, Search, X, Calendar, Lock } from "lucide-react";
...
function InlineAuthPrompt({ onAuthClick }: { onAuthClick: (method: "google" | "apple" | "email") => void }) {
  ...
  const handleEmailSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    onAuthClick("email");
    // Simulate email send - actual implementation will use Supabase
  ...
  if (emailSent) {
    return (
      <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-4 text-center">
        <div className="text-emerald-700 font-medium">Check your email!</div>
  ...
  return (
    <div className="bg-gradient-to-b from-gray-50 to-white border border-gray-200 rounded-xl p-4">
      ...
      <p className="text-xs text-gray-500 mt-3 text-center">
        Free for 7 days &bull; No credit card required
      </p>
    </div>
  );
}
...
export default function TestWorkflowPage() {
  ...
  const handleAuthClick = async (method: "google" | "apple" | "email") => {
    if (method === "google") {
      await supabase.auth.signInWithOAuth({
        provider: "google",
        options: { redirectTo: `${window.location.origin}/testworkflow` },
      });
    ...
    // Email magic link handled in the component
  };
        ...
        {selectedFacility && (
          ...
          >
            <div className="max-h-[60vh] overflow-y-auto">
              ...
              {isYesterday && <InlineAuthPrompt onAuthClick={handleAuthClick} />}
