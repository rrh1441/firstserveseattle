...
export default function TennisCourtList() {
  ...
  useEffect(() => {
    ...
    getTennisCourts()
      .then((data) => {
        ...
        setCourts(data);
        // Note: Keep setIsLoading(false) here AFTER data is set
      })
      .catch((err) => {
  ...
  // --- Handler Functions (toggleFavorite, toggleFilter, toggleMapExpansion, getGoogleMapsUrl) ---
  // (These functions remain the same as before)
    ...
    const getGoogleMapsUrl = (court: Court): string => {
      ...
      // Corrected Google Maps URL pattern
   ...
   // --- Filtering and Sorting ---
    ...
    const filteredCourts = courts.filter((court) => {
        ...
        const matchesFilters = Object.keys(filters).every((key) => {
            ...
            // Check type more carefully - use 'as any' if TS complains about index signature
            return (court as any)[filterKey] === true;
        });
  ...
  // --- Date for Display ---
    ...
    try {
        ...
        todayDate = "Error loading date";
    }


  // --- Render Logic ---

  // *** MODIFIED: Render Skeleton Loader when isLoading ***
  if (isLoading) {
    // Keep the header visible while loading the list part
    return (
